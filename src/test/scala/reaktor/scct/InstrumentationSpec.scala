...
trait InstrumentationSupport {
  ...
  def locateCompiledClasses() = {
    val scalaTargetDir = scalaBinaryVersion
  ...
  def locateScalaJars(): List[File] = List("compiler", "library") map (s =>
    new File("%s/.ivy2/cache/org.scala-lang/scala-%s/jars/scala-%s-%s.jar".format(sys.props("user.home"), s, s, scalaVersion))
  )
